      ...
      <div style={{ background: 'var(--bg-surface)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '16px 20px' }}>
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 16, marginBottom: 8, padding: 0 }}
          >
            <ArrowLeft size={13} />
            시즌 목록으로
          </button>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            <h1 style={{ fontSize: 24, fontWeight: 600, color: 'var(--text-primary)' }}>총력전 대시보드</h1>
            <span style={{ fontSize: 15, fontWeight: 500, padding: '3px 8px', borderRadius: 4, background: 'var(--bg-accent)', color: 'var(--accent)', border: '1px solid var(--border-strong)' }}>
              시즌 {seasonNum}
            </span>
            <span style={{ fontSize: 15, fontWeight: 600, padding: '3px 8px', borderRadius: 4, background: 'var(--bg-surface-2)', color: 'var(--text-muted)', border: '1px solid var(--border)' }}>
              {server === 'jp' ? 'JP' : 'Global'}
            </span>
            {isCurrent && (
              <span style={{ fontSize: 15, fontWeight: 600, padding: '3px 8px', borderRadius: 4, background: 'var(--accent)', color: '#fff' }}>
                진행중
              </span>
            )}
            {currentBoss && (
        ...
        <div style={{ display: 'flex', gap: 16 }}>
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {loading ? (
              <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 10, padding: 48, textAlign: 'center' }}>
                <div style={{ fontSize: 17, color: 'var(--text-muted)' }}>데이터 불러오는 중...</div>
              </div>
            ) : filtered.length === 0 ? (
              <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 10, padding: 48, textAlign: 'center' }}>
                <div style={{ fontSize: 18, color: 'var(--text-primary)', marginBottom: 6 }}>아직 제출된 기록이 없어요</div>
                <div style={{ fontSize: 16, color: 'var(--text-muted)' }}>첫 번째로 클리어 기록을 공유해보세요!</div>
              </div>
            ) : (
              <>
          ...
          <div style={{ width: 200, flexShrink: 0 }}>
            <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px', marginBottom: 12 }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--accent)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 8 }}>
                내 기록 제출
              </div>
              <p style={{ fontSize: 15, color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 10 }}>
              ...
              <button
                ...
                style={{ width: '100%', padding: '7px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-surface-2)', color: 'var(--text-secondary)', fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}
              >
                <RefreshCw size={12} />
                새로고침
              </button>
            ...
            <div style={{ background: 'var(--bg-accent)', border: '1px solid var(--border-strong)', borderRadius: 10, padding: '12px 14px' }}>
              <div style={{ fontSize: 16, fontWeight: 500, color: 'var(--accent-text)', marginBottom: 6 }}>데이터 기여 안내</div>
